@echo off
REM Zen-C BoatRace Game Windows构建脚本
REM 需要安装zc编译器和MinGW

echo Zen-C 赛船大赛游戏 - Windows构建脚本
echo ========================================
...
echo.
echo 正在编译Zen-C代码...
zc build -o build\boatrace_game.exe src\main_window.zc

if %errorlevel% equ 0 (
    echo 编译成功!
    echo.
    echo 运行游戏:
    echo build\boatrace_game.exe
    echo.
    echo 或者直接运行:
    echo boatrace_game.exe
) else (
    echo 编译失败!
    echo 请检查错误信息
)
