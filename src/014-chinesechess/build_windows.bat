@echo off
REM Zen-C 中国象棋 Windows 构建脚本
REM 需要安装 zc 编译器和 MinGW

echo Zen-C 中国象棋 - Windows 构建脚本
echo ========================================
...
echo.
echo 正在编译 Zen-C 代码...
zc build -o build\chinese_chess.exe src\main_window.zc

if %errorlevel% equ 0 (
    echo 编译成功!
    echo.
    echo 运行游戏:
    echo build\chinese_chess.exe
) else (
    echo 编译失败!
    echo 请检查错误信息
)
